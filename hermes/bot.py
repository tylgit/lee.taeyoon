...
텔레그램으로 질문을 보내면 Claude가 답하고, 답변을 글과 함께
한국어 음성(TTS)으로도 보내 줍니다. 시니어 사용자를 위해
...
def ask_claude(chat_id: int, question: str) -> str:
    """대화 기록을 이어서 Claude에게 질문하고 답변 텍스트를 돌려줍니다."""
    ...
    if response.stop_reason == "refusal":
        history.pop()
        return "죄송합니다. 이 질문에는 답변드리기 어렵습니다. 다른 질문을 해 주시겠어요?"

    answer = "".join(b.text for b in response.content if b.type == "text").strip()
    if not answer:
        history.pop()
        return "죄송합니다. 답변을 만들지 못했습니다. 다시 한 번 질문해 주세요."

    history.append({"role": "assistant", "content": answer})
    # 오래된 대화는 잘라내어 비용과 속도를 관리합니다.
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        answer = await asyncio.to_thread(ask_claude, chat.id, question)
    except anthropic.APIConnectionError:
        await update.message.reply_text("인터넷 연결에 문제가 있습니다. 잠시 후 다시 시도해 주세요.")
        return
    except anthropic.RateLimitError:
        await update.message.reply_text("지금 이용자가 많습니다. 잠시 후 다시 질문해 주세요.")
        return
    except anthropic.APIStatusError as e:
        logger.error("Claude API 오류: %s", e)
        await update.message.reply_text("답변 중 문제가 생겼습니다. 잠시 후 다시 시도해 주세요.")
        return
...
def main() -> None:
    ...
    if not os.environ.get("ANTHROPIC_API_KEY"):
        logger.warning("ANTHROPIC_API_KEY가 없습니다. `ant auth login` 프로필이 있으면 그대로 사용됩니다.")
